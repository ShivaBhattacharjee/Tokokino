...
export type ElasticSliderProps = {
  ...
  /**
   * Called when the user finishes a pointer interaction (release) or a discrete
   * keyboard nudge. Useful for commit-on-release + live-preview patterns.
   */
...
export function ElasticSlider({
  ...
  /** Latest value during a pointer session so commit always sees the final pick. */
  const sessionValueRef = useRef(value)

  const percentage = ((value - min) / (max - min)) * 100
  // Always show the expanded track chrome (handle, hash marks, active fill).
  // Hover/drag still drive handle opacity intensity via isDragging / isHovered.
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      sessionValueRef.current = rounded
      setValue(rounded)
    },
    [
      disabled,
  ...
  const handlePointerUp = useCallback(
    (e: React.PointerEvent) => {
      ...
      let committed = sessionValueRef.current
  ...
  const discreteSteps = (max - min) / step
  const hashMarkCount = discreteSteps <= 10 ? Math.max(0, discreteSteps - 1) : 9
  ...
  return (
    <div
      ...
      style={
        {
          // Neutral track/ticks/label; fill + handle use strawberry primary.
          "--elastic-slider-bg": "var(--muted)",
          "--elastic-slider-fill":
            "color-mix(in oklab, var(--primary) 22%, transparent)",
          "--elastic-slider-fill-active":
