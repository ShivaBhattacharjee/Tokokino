...
export function NotFoundArtwork() {
  ...
  useEffect(() => {
    ...
    // Stay near the diagonal letterforms so each destination lights an edge.
    let horizontal: ReturnType<typeof animate> | undefined
    let vertical: ReturnType<typeof animate> | undefined
    const wander = () => {
      const nextX = 60 + Math.random() * 395
      const nextY = 425 - nextX * 0.65 + (Math.random() - 0.5) * 125
      horizontal = animate(x, nextX, { duration: 3.6, ease: "easeInOut" })
      vertical = animate(y, nextY, { duration: 3.6, ease: "easeInOut" })
    }
    // Let the first 4 catch the light before the idle glow drifts away.
    let interval: number | undefined
    const delay = window.setTimeout(() => {
      wander()
      interval = window.setInterval(wander, 4000)
    }, 1400)
    return () => {
  ...
  return (
    ...
    >
      <defs>
        ...
        >
          <stop className="[stop-color:#fafafa] dark:[stop-color:#090909]" />
          <stop
            offset="1"
            className="[stop-color:#eaeaea] dark:[stop-color:#161616]"
          />
        </linearGradient>
        <linearGradient
          id={`${id}-color`}
        ...
        >
          ...
          <stop offset="0.85" stopColor="var(--accent-foreground)" />
        </linearGradient>
      ...
      {FACES.map((path, index) => (
        <path
          key={index}
          d={path}
          fill={index % 2 ? `url(#${id}-face)` : undefined}
          className={
            index % 2 ? undefined : "fill-neutral-100 dark:fill-[#090909]"
          }
        />
      ))}
      <use
        href={`#${id}-edges`}
        className="fill-neutral-300 dark:fill-[#303030]"
      />
      <g mask={`url(#${id}-mask)`}>
        <use href={`#${id}-edges`} fill={`url(#${id}-color)`} />
        <use
          href={`#${id}-edges`}
          fill={`url(#${id}-color)`}
