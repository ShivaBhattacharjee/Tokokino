...
export function curlForEndpoint(method: string, path: string): string {
  ...
  switch (`${method} ${path}`) {
    ...
    case "POST /api/drafts":
      return json(
        '{"name": "Landing hero", "state": {"schemaVersion": 1, "present": {"canvases": [{"id": "canvas-1"}]}}}'
      )
    case "PATCH /api/drafts/{id}":
      return json('{"name": "Landing hero v2"}')
    case "POST /api/presets":
      return json(
        '{"name": "Hero split", "type": "style", "geometry": {"canvasTilt": {"rx": 0, "ry": 0, "rz": 0}, "canvasScale": 100, "slots": []}}'
      )
